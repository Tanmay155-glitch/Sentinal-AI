...
// Helper to add a content card container
function addCard(slide, x, y, w, h, title, bullets, borderColor = COLOR_CARD_BORDER) {
  ...
  if (bullets && bullets.length > 0) {
    const textObjects = bullets.map(b => ({
      text: b.bold ? `${b.bold}: ` : "",
      options: { bold: true, color: COLOR_NAVY_DARK }
    })).flatMap((item, index) => [
      { text: bullets[index].bold ? `${bullets[index].bold}: ` : "", options: { bold: true, color: COLOR_NAVY_DARK, fontSize: 10.5, fontFace: "Arial" } },
      { text: `${bullets[index].text}\n`, options: { bold: false, color: COLOR_TEXT_MAIN, fontSize: 10.5, fontFace: "Arial" } }
    ]);
...
slide1.addText("CAPSTONE PROJECT & INDUSTRY EVALUATION", {
  x: 1.0, y: 0.8, w: 11.33, h: 0.4,
  fontSize: 12, bold: true, color: COLOR_BLUE_PRIMARY, align: "center", fontFace: "Arial"
});

slide1.addText("SENTINEL AI", {
  x: 1.0, y: 1.3, w: 11.33, h: 1.0,
...
// Presenter Cards on Title Slide
const teamMembers = [
...
teamMembers.forEach((m, idx) => {
  const cardX = 1.0 + idx * 2.85;
  slide1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: cardX, y: 3.8, w: 2.65, h: 1.2,
    fill: { color: "1E293B" }, line: { color: "334155", width: 1 }, rectRadius: 0.1
  });
  slide1.addText(m.name, {
    x: cardX, y: 4.0, w: 2.65, h: 0.35,
    fontSize: 13, bold: true, color: "FFFFFF", align: "center", fontFace: "Arial"
  });
  slide1.addText(m.role, {
    x: cardX, y: 4.4, w: 2.65, h: 0.4,
    fontSize: 10, color: "94A3B8", align: "center", fontFace: "Arial"
  });
});
...
addCard(slide2, 0.6, 1.5, 5.8, 3.6, "⚠️ Current Industry Vulnerabilities", [
...
addCard(slide2, 6.8, 1.5, 5.9, 3.6, "🎯 The Sentinel AI Solution", [
...
owners.forEach((o, idx) => {
  ...
  const cx = 0.6 + col * 6.1;
  const cy = 1.5 + row * 1.85;
  addCard(slide3, cx, cy, 5.8, 1.7, `${o.name} — ${o.title}`, o.bullets);
});
...
addCard(slide4, 0.6, 1.5, 3.8, 3.6, "⚛️ Desktop Shell & UI", [
...
addCard(slide4, 4.7, 1.5, 3.8, 3.6, "🦀 Native Rust Core Engine", [
...
addCard(slide4, 8.8, 1.5, 3.9, 3.6, "🧠 Machine Learning & Storage", [
...
addCard(slide5, 0.6, 1.5, 5.8, 3.6, "⚡ Continuous Telemetry Collection", [
...
addCard(slide5, 6.8, 1.5, 5.9, 3.6, "🛡️ Behavioral Heuristic Rules", [
...
addCard(slide6, 0.6, 1.5, 5.8, 3.6, "🤖 Model Training & Architecture", [
...
addCard(slide6, 6.8, 1.5, 5.9, 3.6, "📈 Empirical Performance & Explainability", [
...
addCard(slide7, 0.6, 1.5, 5.8, 3.6, "🌐 Network Flow Correlation", [
...
addCard(slide7, 6.8, 1.5, 5.9, 3.6, "📡 Subnet Device Discovery", [
...
addCard(slide8, 0.6, 1.5, 5.8, 3.6, "🔑 Windows Registry Inspection", [
...
addCard(slide8, 6.8, 1.5, 5.9, 3.6, "📁 Startup Folder & MITRE Mapping", [
...
addCard(slide9, 0.6, 1.5, 5.8, 3.6, "🏷️ MITRE ATT&CK Alert Matrix", [
...
addCard(slide9, 6.8, 1.5, 5.9, 3.6, "📄 NIST SP 800-61 Rev. 2 Exporter", [
...
uiCards.forEach((c, idx) => {
  ...
  const cx = 0.6 + col * 6.1;
  const cy = 1.5 + row * 1.85;
  addCard(slide10, cx, cy, 5.8, 1.7, c.title, [{ bold: "Functionality", text: c.text }]);
});
...
addCard(slide11, 0.6, 1.5, 3.8, 3.6, "⚡ CPU & RAM Overhead", [
...
addCard(slide11, 4.7, 1.5, 3.8, 3.6, "⏱️ Inference & Probing Latency", [
...
addCard(slide11, 8.8, 1.5, 3.9, 3.6, "🔒 Security & Operational Safety", [
...
addCard(slide12, 0.6, 1.5, 5.8, 3.6, "🚀 Phase 1: Near-Term Enhancements", [
...
addCard(slide12, 6.8, 1.5, 5.9, 3.6, "🌐 Phase 2: Enterprise Infrastructure", [
...
slide13.addText("PROJECT COMPLETION & EVALUATION", {
  x: 1.0, y: 0.8, w: 11.33, h: 0.4,
  fontSize: 12, bold: true, color: COLOR_GREEN_ACCENT, align: "center", fontFace: "Arial"
});
...
slide13.addText("Presenters: Tanmay Gawali (Lead)  |  Divij Kulkarni  |  Soham Wadale  |  Ananya Kaushik", {
  x: 1.0, y: 4.5, w: 11.33, h: 0.4,
  fontSize: 12, color: "CBD5E1", align: "center", fontFace: "Arial"
});
...
pptx.writeFile({ fileName: outputPath }).then(fileName => {
  console.log("SUCCESS: PPTX Presentation created at " + fileName);
}).catch(err => {
